...
class Visualizer:
    def tsplot(self, x, name, dir):
        ...
        plt.plot(x, '-r*', ms=0.01, linewidth=1)
    ...
    def mtsplot(self, y, name, dest, dir):
        ...
        plt.clf()
        plt.figure(figsize=(6, 4), dpi=500, facecolor='white')
        colors = ["#E24A33", "#348ABD", "#988ED5", "#ac02ab", "#aba808", "#05aaa8", "#151515", "#94a169", "#bec9cd",
                  "#6a6551"]
        # E24A33 : red
        # 348ABD : blue
        # 988ED5 : purple
        ...
        for n in range(y.shape[1]):
            plt.plot(y[:, n], c=colors[n], linewidth=1, marker='*', markersize=0.001)
        plt.legend(name, fontsize=10)
        plt.xlabel('Time /2h', fontsize=9)
        plt.ylabel(dest + ' value', fontsize=9)
        plt.title(dest)
        plt.savefig('{}/{}.png'.format(dir, dest))
        plt.close()
